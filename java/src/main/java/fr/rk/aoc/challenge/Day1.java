...
public final class Day1 {
     ...
     * @return number of times a values increased comparing to previous one
     */
    public static long nbTimesSumValuesIncreased(List<Long> values) {
        return IntStream.range(0, values.size()-3)
                .reduce(0, (partial, current) ->
                        partial + ( (values.get(current)+values.get(current+1) + values.get(current+2)) < (values.get(current+1) + values.get(current+2) + values.get(current+3)) ? 1 : 0));
